			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (NetWorkUtils.checkNetState(getActivity())) {
					...
					if (getActivity() instanceof MainActivity) {
						...
						MoreDetailInfoFragment moreDetailInfoFragment = new MoreDetailInfoFragment(
								baseUrl, newsId, flag, news);
						((MainActivity) getActivity())
								.switchFragment(moreDetailInfoFragment);
					}
				} else {
	...
	private void loadingMore(final int itemCount) {
		if (NetWorkUtils.checkNetState(getActivity())) {
			new Thread(new Runnable() {
				...
				@Override
				public void run() {
					List<News> newListsMore = paseHtml(GlobalParam.OTHER_FIRST
							+ "?u&start=" + itemCount);
